...
public:
	...
	std::allocator<Header> allocator;
	...
	_Myt& replace(iterator _F1, iterator _L1,
		...
		size_type _M = std::distance(_F2, _L2);
...
private:
	...
	void _Copy( size_type _N )
	{
		...
		Header* header = allocator.allocate( _N + HEADER_SIZE_ELEMENTS + 1, 0 );
		header->m_RefCount = 0;
		header->m_Reserve  = _N;
...
#				endif // GPSTRING_TRACKING
				...
				allocator.deallocate( header, header->m_Reserve + HEADER_SIZE_ELEMENTS + 1 );
			}
			else
			{
				--header->m_RefCount;
...
template<class _E, class _Tr, class _A> inline
    ...
    if (_Ok)
	{
        ...
        //_TRY_IO_BEGIN
        typename _A::size_type _N = 0 < _I.width() ? _I.width() : _X.max_size();
